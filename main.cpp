...
bool shadowFeeler(vec4 p0, Object *object){
  ...
  //p0 -> P_w
  // trace ray from intersection point to the lightPosition and if hits something then inShadow = true
  //loop thorugh all the objects
  //test intersections with all of them like in castray
  //hits at least one than in shadow, return t_w
  //cut off ray at the point light  t_w < length(L-P) then valid and inShadow

  vec4 L = lightPosition - p0;
  double rayLength = length(L);
  L = normalize(L);

  for (unsigned int i = 0; i < sceneObjects.size();i++) {
	  Object::IntersectionValues shadowResults = sceneObjects[i]->intersect(p0, L);
	  if (shadowResults.t_w < rayLength) { return true; }
  }
...
/* -------------------------------------------------------------------------- */
vec4 getColor(Object *obj, Object::IntersectionValues results, vec4 E) {
	vec4 color = vec4(1.0, 0.0, 0.0, 1.0);

	//vector computer from the point on the object from the direction of the light

	vec4 pos = results.P_w;
	vec4 L = normalize(lightPosition - pos); L.w = 0.0;
	vec4 V = normalize(E); V.w = 0.0;					
	vec4 R = normalize(reflect(L, results.N_w));		 


	//ambient//////////////
	color4 material_ambient = color4(	obj->shadingValues.color.x*obj->shadingValues.Ka,
										obj->shadingValues.color.y*obj->shadingValues.Ka,
										obj->shadingValues.color.z*obj->shadingValues.Ka, 1.0);
	vec4 ambient = GLState::light_ambient * material_ambient;

	//diffuse/////////////
	float Kd = max(dot(L, results.N_w), 0.0);
	color4 material_diffuse = color4(	obj->shadingValues.color.x, 
										obj->shadingValues.color.y, 
										obj->shadingValues.color.z, 1.0);
	vec4  diffuse = Kd * GLState::light_diffuse * material_diffuse;

	//specular///////////
	float shininess = obj->shadingValues.Kn;
	float Ks = pow(max(dot(V, R), 0.0), shininess);
	color4 material_specular = color4(	obj->shadingValues.Ks,
										obj->shadingValues.Ks,
										obj->shadingValues.Ks, 1.0);
	vec4  specular = Ks * GLState::light_specular * material_specular;


	//set color
	if (dot(L, results.N_w) < 0.0) {
		color = vec4(0.0, 0.0, 0.0, 1.0);
	}
	else {
		color = ambient + diffuse + specular;
	}
	//color is additive -- color values no greater than 1.0 (white)
...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth){
  ...
  if (results[0].t_w != std::numeric_limits< double >::infinity()) {
	  ...
	  if (shadowFeeler(results[0].P_w, NULL)) {		//in shadows
		  color = vec4(0.0, 0.0, 0.0, 1.0);
	  }
	  else {	//else phong shade with getColor()
		  color = getColor(sceneObjects[results[0].ID_], results[0], E);
	  }
  }
...
void initUnitSphere(){
  ...
  {
  sceneObjects.push_back(new Sphere("Green Diffuse Sphere"));
  Object::ShadingValues _shadingValues;
  _shadingValues.color = vec4(0.0,1.0,0.0,1.0);
  _shadingValues.Ka = 0.0;
  _shadingValues.Kd = 1.0;
  _shadingValues.Ks = 1.0; //changed from 0.0 to check specular
  _shadingValues.Kn = 5.0; // changed from 16.0 to check specular
  _shadingValues.Kt = 0.0;
  _shadingValues.Kr = 0.0;
  sceneObjects[sceneObjects.size()-1]->setShadingValues(_shadingValues);
 ...
 /* {
	  sceneObjects.push_back(new Sphere("Blue Diffuse Sphere"));
	  Object::ShadingValues _shadingValues;
...
void initUnitSquare(){
  ...
  { //Back Wall
    ...
    _shadingValues.Kr = 0.0;
    sceneObjects[sceneObjects.size()-1]->setShadingValues(_shadingValues);
    sceneObjects[sceneObjects.size()-1]->setModelView(RotateZ(45));
  }
